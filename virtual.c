...
int count_page_faults_lfu(struct PTE page_table[TABLEMAX], int table_cnt, int refrence_string[REFERENCEMAX], int reference_cnt, int frame_pool[POOLMAX], int frame_cnt)
{
    ...
    for (int i = 0; i < reference_cnt; i++)
    {
        int frame_number_1 = page_table[refrence_string[i]].frame_number;
        int current_timestamp = i + 1;
        int frame_number = process_page_access_lfu(page_table, &table_cnt, refrence_string[i], frame_pool, &frame_cnt, current_timestamp);
        if (frame_number_1 != frame_number)
        {
            page_faults++;
        }
    }
