...
int process_page_access_fifo(struct PTE page_table[TABLEMAX], int *table_cnt, int page_number, int frame_pool[POOLMAX], int *frame_cnt, int current_timestamp)
{
    ...
    {
        int freed_frame_number = page_table[victim_page_number].frame_number;
        page_table[victim_page_number].is_valid = 0;
        page_table[victim_page_number].frame_number = -1;
        page_table[victim_page_number].arrival_timestamp = -1;
        page_table[victim_page_number].last_access_timestamp = -1;
        page_table[victim_page_number].reference_count = -1;
...
int count_page_faults_fifo(struct PTE page_table[TABLEMAX], int table_cnt, int refrence_string[REFERENCEMAX], int reference_cnt, int frame_pool[POOLMAX], int frame_cnt)
{
    ...
    for (int i = 0; i < reference_cnt; ++i)
    {
        ...
        if (!frame_cnt)
            time++;
        process_page_access_fifo(page_table, &table_cnt, page_number, frame_pool, &frame_cnt, time);
    }
    return count;
}
